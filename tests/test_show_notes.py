...
class TestShowNotesDownload:
    ...
    @pytest.mark.asyncio
    async def test_downloader_show_notes_integration(self, test_data_manager, test_download_dir, sample_urls, http_mocker):
...
class TestShowNotesQuality:
    ...
    @pytest.mark.asyncio
    async def test_content_completeness_comparison(self, test_data_manager, sample_urls):
        """测试内容完整性对比"""
        parser = JsonScriptParser()
        ...
        found_phrases = [phrase for phrase in key_phrases if phrase in parsed_show_notes]
    ...
    async def test_multilingual_content_handling(self, test_data_manager, sample_urls):
        ...
        for i, test_url in enumerate(sample_urls):
            ...
            # 验证中文内容处理
            chinese_chars = sum(1 for char in show_notes if '\u4e00' <= char <= '\u9fff')
            english_chars = sum(1 for char in show_notes if char.isalpha() and ord(char) < 256)
            
            print(f"\nURL {i+1}: {episode_info.title}")
            print(f"中文字符: {chinese_chars}, 英文字符: {english_chars}")
            
            assert chinese_chars > 100  # 应该包含大量中文
            assert len(show_notes.encode('utf-8')) > len(show_notes)  # UTF-8编码长度应该更大
