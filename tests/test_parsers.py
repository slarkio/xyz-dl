...
class TestJsonScriptParserOffline:
    ...
    @pytest.mark.asyncio
    async def test_parse_episode_info_with_real_data(self, test_data_manager, sample_urls):
    ...
    @pytest.mark.asyncio
    async def test_extract_audio_url_with_real_data(self, test_data_manager, sample_urls):
        """测试使用真实HTML数据提取音频URL"""
        parser = JsonScriptParser()
    ...
    @pytest.mark.asyncio 
    async def test_show_notes_extraction_completeness(self, test_data_manager, sample_urls):
        """测试Show Notes提取的完整性"""
        parser = JsonScriptParser()
        ...
        for i, test_url in enumerate(sample_urls[:2]):  # 测试前两个URL
            ...
            assert "时间轴" in episode_info.shownotes or "【" in episode_info.shownotes  # 应包含典型内容
            assert len(episode_info.shownotes) >= len(html_show_notes) * 0.8  # 应该使用了HTML提取的内容
